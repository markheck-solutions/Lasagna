...
| `KEEP` | `TRANSPORT_DEVICE_ADJACENCY` | Keep as same-site / transport continuity authority. | Sorter consumes only `ENDPOINT_PROOF_SOURCE = EXACT_DEVICE_PORT_MATCH`. |
| `KEEP` | `PORT_MATCH_RULE` | Keep as structured proof discriminator. | DWDM cabling proof uses `CABLING_POINT_TO_PEER_CABLING_POINT`; source classifier labels remain separate. |
| `REPLACE_WITH_CABLING_PROOF` | DTN adjacency from broad device/content fanout | Replace with cabling-backed Snowflake path only. | SQL excludes generic DTN endpoint inference and emits DTN only through nonblank `CABPT_INT_ID` plus cabling join. |
...
| `src/lasagna/route_sorting/combined_results.py` / Snowflake SQL | `TL_DEVICE` / `TL_DEVICE_SHARED_HANDOFF` | Shallow device adjacency | Deleted from trusted sorter and removed from current export. | `ENDPOINT_*_DEVICE_SLOT`, `ENDPOINT_*_DEVICE_SUBSLOT`, `ENDPOINT_*_CCP_CONNECTION_POINT_NR`, and structured `PORT_MATCH_RULE`. | Missing, duplicate, ambiguous, unsupported, or untrusted port-level endpoint proof. |
...
- Full-route verifier status is `OK`: each successful service matches the expected full-route row sequence and the failed service has an approved fail-closed message.
