...
def test_full_route_verifier_allows_expected_transport_fail_closed_service(
    tmp_path: Path,
) -> None:
    parsed_inputs = parse_service_id_text("IC-123456")
    service_results = {
...
def test_full_route_verifier_ignores_visible_failed_source_rows_as_proof(
    tmp_path: Path,
) -> None:
    parsed_inputs = parse_service_id_text("IC-123456")
    service_results = {
        "IC-123456": ServiceRouteResult.sort_failed(
            ...
            "Source rows shown for troubleshooting only; not route proof.",
            (_route_row("AAA", "AAA-BBB OL01", "1"),),
    ...
    assert expected["services"]["IC-123456"]["rows"] == []
    assert result["status"] == "OK"
    assert result["services"][0]["workbook_status"] == "SORT FAILED"
    assert result["services"][0]["route_order_source"] == "SOURCE_ROWS_NOT_ROUTE_PROOF"
    assert result["services"][0]["actual_row_count"] == 0
    assert result["verifier_failures"] == []
...
def test_full_route_verifier_compares_migration_rows(tmp_path: Path) -> None:
    parsed_inputs = parse_service_id_text("IC-123456")
    expected_results = {
        "IC-123456": ServiceRouteResult.ok(
            "IC-123456",
            (_route_row("AAA", "AAA-BBB OL01", "1"),),
            (_route_row("CCC", "CCC-DDD OL02", "2"),),
            route_order_source=ROUTE_ORDER_AUTHORITY,
        )
    }
    ...
    expected = capture_expectations(expected_workbook.path)
    assert len(expected["services"]["IC-123456"]["rows"]) == 2
    ...
    assert result["services"][0]["expected_row_count"] == 2
    assert result["services"][0]["actual_row_count"] == 1
