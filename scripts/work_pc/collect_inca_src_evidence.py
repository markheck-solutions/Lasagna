...
def main() -> int:
    ...
    write_command_log(run_dir, args.database, args.schema)
    ...
    try:
        ...
        try:
            ...
            artifacts = collect_live_artifacts(cursor, config, metadata)
        finally:
            cursor.close()
    finally:
        connection.close()
    write_artifacts(run_dir, config, metadata, artifacts)
    ...
    for anchor in anchor_profiles(profiles):
        ...
        try:
            count, query_id = execute_count(
                cursor, build_anchor_count_sql(config, anchor), (config.service_id,)
            )
        ...
        fetched = fetch_anchor_pages(cursor, config, anchor, object_proof, pages)
        seed_rows.extend(
...
def scan_single_predicate(
    ...
    try:
        count, query_id = execute_count(
            cursor, predicate_sql, (predicate_value(node, predicate_column),)
        )
    except Exception as exc:
        skipped_rows.append(
    ...
    if pages > config.max_pages_per_predicate:
        ...
        pages = config.max_pages_per_predicate
    fetched_rows = fetch_exact_pages(cursor, config, predicate_column, proof_columns, node, pages)
...
def execute_rows(cursor: object, sql_text: str, params: tuple[object, ...]) -> QueryRows:
    execute = getattr(cursor, "execute")
...
def execute_count(cursor: object, sql_text: str, params: tuple[object, ...]) -> tuple[int, str]:
    result = execute_rows(cursor, sql_text, params)
    if not result.rows:
...
def fetch_anchor_pages(
    ...
    for page_number in range(1, pages + 1):
        ...
        result = execute_rows(cursor, sql_text, (config.service_id, config.page_size, offset))
